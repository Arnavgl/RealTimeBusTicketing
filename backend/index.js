...
// File: backend/index.js (The Final, Corrected Version)
...
wss.on("connection", (ws, req) => {
  ...
  ws.on("close", () => console.log(`Client disconnected: ${ws.sessionId}`));
});

function broadcast(data, originatorSessionId) {
  wss.clients.forEach((client) => {
    if (
      client.readyState === WebSocket.OPEN &&
      client.sessionId !== originatorSessionId
    ) {
      client.send(JSON.stringify(data));
    }
  });
...
// --- ----------------- ---
// --- ALL API ROUTES MUST BE DEFINED FIRST ---
// --- ----------------- ---
...
app.post("/api/seats/hold", async (req, res) => {
  ...
  try {
    ...
    // NOTE: Using uppercase EX/NX as you confirmed it works for you.
    const result = await redisClient.set(holdKey, userId, {
      EX: HOLD_DURATION_SECONDS,
      NX: true,
    });
    if (result) {
      seat.status = "held";
      await seat.save();
      broadcast({ type: "SEAT_UPDATE", payload: seat }, sessionId);
      res.status(200).json({ message: "Seat held successfully.", seat });
    } else {
      res
        .status(409)
...
app.post("/api/seats/purchase", async (req, res) => {
  const { seatIds, email, sessionId } = req.body;
  const userId = "user123";
  try {
    let purchasedSeatsInfo;
    const tripInfo = await sequelize.transaction(async (t) => {
      const seatsToUpdate = [];
      for (const seatId of seatIds) {
        const holdKey = `seat:${seatId}:hold`;
        const heldBy = await redisClient.get(holdKey);
        ...
        if (seat.status !== "held")
          throw new Error(`Seat ${seatId} is not in a held state.`);
        seat.status = "sold";
        seatsToUpdate.push(seat.save({ transaction: t }));
      }
      purchasedSeatsInfo = await Promise.all(seatsToUpdate);
      for (const seatId of seatIds) {
        await redisClient.del(`seat:${seatId}:hold`);
      }
      return await Trip.findByPk(purchasedSeatsInfo[0].tripId, {
        transaction: t,
      });
    });
    ...
    purchasedSeatsInfo.forEach((seat) => {
      broadcast({ type: "SEAT_UPDATE", payload: seat }, sessionId);
    });
...
// --- ------------------------------------- ---
// --- SERVE FRONTEND (This must be the final part) ---
// --- ------------------------------------- ---
